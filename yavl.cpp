...
namespace YAVL {
template <> std::string ctype2str<unsigned long long>()
{
    return "unsigned long long";
}

template <> std::string ctype2str<std::string>()
{
    return "string";
}

template <> std::string ctype2str<long long>()
{
    return "long long";
}

template <> std::string ctype2str<unsigned int>()
{
    return "unsigned int";
}

template <> std::string ctype2str<int>()
{
    return "int";
}

template <> std::string ctype2str<bool>()
{
    return "bool";
}

template <> std::string ctype2str<float>()
{
    return "float";
}
...
bool Validator::validate_map(const YAML::Node& mapNode, const YAML::Node& document)
{
    if (document.Type() != YAML::NodeType::Map) {
...
bool Validator::validate_leaf(const YAML::Node& gr, const YAML::Node& document)
{
    ...
    if (type == "string") {
        ...
        if (!ok) {
            std::string reason = "enum std::string '" + docValue + "' is not allowed.";
            gen_error(Exception(reason, gr_path, doc_path));
        }
    } else if (type == "bool") {
        attempt_to_convert<bool>(document, ok);
    } else if (type == "float") {
        attempt_to_convert<float>(document, ok);
    }
    return ok;
}
